...
function adminUyeDetay(eKey){
    ...
    html+='</div>';
    // Spora başlama amacı
    if(u.amaclar && u.amaclar.length>0){
        html+='<div style="margin-top:10px;padding:10px;background:var(--bg2);border:1px solid var(--border);border-radius:8px;">';
        html+='<div style="font-size:11px;color:var(--accent);font-weight:700;margin-bottom:6px;">🎯 Spora Başlama Amacı</div>';
        u.amaclar.forEach(function(a){
            html+='<div style="font-size:12px;color:var(--text2);padding:2px 0;">• '+esc(a)+'</div>';
        });
        html+='</div>';
    }
...
function adminUyeEkleModal(prefill){
    ...
    // Spora başlama amacı
    html+='<div class="form-group"><label class="form-label">🎯 Spora Başlama Amacı</label>';
    html+='<div id="ue-amaclar-liste">';
    var mevcutAmaclar = p.amaclar || [];
    mevcutAmaclar.forEach(function(a,i){
        html+='<div style="display:flex;gap:4px;margin-bottom:4px;align-items:center;"><input type="text" class="input ue-amac-input" value="'+esc(a)+'" style="padding:8px 10px;font-size:13px;"><button class="btn btn-danger btn-sm" style="padding:4px 8px;" onclick="this.parentElement.remove()">✕</button></div>';
    });
    html+='</div>';
    html+='<div style="display:flex;gap:4px;"><input type="text" id="ue-amac-yeni" class="input" placeholder="Örn: Kilo vermek, Kas yapmak..." style="padding:8px 10px;font-size:13px;" onkeydown="if(event.key===\'Enter\'){event.preventDefault();ueAmacEkle();}"><button class="btn btn-ghost btn-sm" onclick="ueAmacEkle()">+ Ekle</button></div>';
    html+='</div>';
...
function ueAmacEkle(){
    var input = document.getElementById('ue-amac-yeni');
    var val = input.value.trim();
    if(!val) return;
    var liste = document.getElementById('ue-amaclar-liste');
    liste.insertAdjacentHTML('beforeend','<div style="display:flex;gap:4px;margin-bottom:4px;align-items:center;"><input type="text" class="input ue-amac-input" value="'+esc(val)+'" style="padding:8px 10px;font-size:13px;"><button class="btn btn-danger btn-sm" style="padding:4px 8px;" onclick="this.parentElement.remove()">✕</button></div>');
    input.value = '';
    input.focus();
}
window.ueAmacEkle = ueAmacEkle;
...
async function adminUyeKaydet(){
    ...
    // Amaçları topla
    var amaclar=[];
    document.querySelectorAll('.ue-amac-input').forEach(function(inp){ var v=inp.value.trim(); if(v) amaclar.push(v); });
    ...
    try{
        ...
        // Admin verisine ekle
        G.adminData.uyeler[eKey]={ad:ad,soyad:soyad,tel:tel,adres:adres,sifre:sifre,amaclar:amaclar,uyelikBaslangic:baslangic,uyelikBitis:bitisStr,uyelikAy:surAy,kayitTarihi:bugunStr(),avatar:'👤'};
        await fbYaz('admin',G.adminData);
...
function adminArsivdenEkle(eKey){
    ...
    var u=a.uyeBilgi||{};
    adminUyeEkleModal({ad:u.ad,soyad:u.soyad,email:keyToEmail(eKey),tel:u.tel,adres:u.adres,amaclar:u.amaclar||[]});
}
